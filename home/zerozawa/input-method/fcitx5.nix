{
  ...
  xdg.dataFile = {
    ...
    "fcitx5/rime/rime_ice.custom.yaml".source = (pkgs.formats.yaml {}).generate "rime_ice.custom.yaml" {
      patch = {
          ...
          (pkgs.fcitx5-rime.override
            {
              ...
              rimeDataPkgs =
                [
                  pkgs.rime-japanese
                  (pkgs.nur.repos.Freed-Wu.rime-kaomoji.overrideAttrs
                    (old: {
                        ...
                        postInstall = lib.concatMapStrings (x: x + "\n") [
                          ...
                          in
                            lib.concatMapStrings (dict_file: ''
                              sed -i '/^\.\.\./i\  - ${dict_file}' $out/share/rime-data/rime_ice.dict.yaml
                            '')
                            dict_files)
                          dicts_to_added)
